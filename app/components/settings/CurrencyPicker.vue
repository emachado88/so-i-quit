...
<template>
  ...
  >
    <div class="flex max-h-[75vh] w-full max-w-sm flex-col rounded-2xl bg-surface p-5 shadow-xl">
      <h3 class="text-lg font-bold text-ink">{{ t('settings.currency') }}</h3>
      <input
        id="currency-search"
        v-model="search"
        type="search"
      ...
      <div class="mt-2 flex flex-1 flex-col overflow-y-auto">
        <button
          ...
          :class="option.code === current ? 'bg-primary-soft text-on-primary-soft' : ''"
        ...
        >
          ...
          <span v-if="option.code === current" class="text-primary">✓</span>
        </button>
        <p v-if="options.length === 0" class="px-3 py-4 text-center text-sm text-muted">
