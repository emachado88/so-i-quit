...
const breakdownLabel = computed(() => {
  ...
  const pieces: string[] = []
  if (years) {
    pieces.push(`${years} ${t(years === 1 ? 'progress.year' : 'progress.years')}`)
  }
  if (months) {
    pieces.push(
  ...
  if (hours && days.value > 0) {
    pieces.push(`${hours} ${t(hours === 1 ? 'progress.hour' : 'progress.hours')}`)
  }
  return pieces.join(' · ')
})
</script>
...
<template>
  <article class="relative overflow-hidden rounded-2xl border border-border bg-surface shadow-sm">
    <div class="absolute inset-y-0 left-0 w-[5px] bg-gradient-to-b from-primary to-accent" />
    <div class="flex items-center gap-3.5 p-4 pl-5">
      <div class="min-w-0 flex-1">
        ...
        <p v-if="!justStarted" class="mt-0.5 text-[44px] font-black leading-[1.05] tracking-tight text-ink">
          {{ bigValue }}
          <span class="ml-1.5 text-base font-bold text-muted">
            {{ t(bigUnitKey) }}
          </span>
        </p>
      ...
      <div class="flex shrink-0 flex-col items-center gap-1.5">
        ...
        <span class="text-[11px] font-extrabold text-primary">{{ percent }}%</span>
      </div>
    </div>
    <div class="chips flex gap-1.5 overflow-x-auto px-4 pb-3 [scrollbar-width:none]">
      <span
