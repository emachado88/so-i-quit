...
- **Capacitor 8** — `@capacitor/core|cli|android|ios|app|local-notifications|haptics`; `android/` + `ios/` are committed; `dist/` (cloudflare_pages preset output) is the webDir. **iOS uses Swift Package Manager** (CapApp-SPM `Package.swift` references the plugins — no Podfile)
...
ios/                       # Capacitor iOS project (committed; SPM — no Podfile)
  ...
  App/App/Info.plist       # CFBundleDisplayName "So I Quit"; MARKETING_VERSION 1.0.0-beta
...
- **`.github/workflows/mobile-preview.yml`** — CD preview installers, `workflow_dispatch` only (previews are on-demand — `ci` already gates PRs and macOS runner minutes are expensive): **android** job (ubuntu) builds `assemblePreview` APK (debug-signed, sideloadable); **ios** job (macos-15) builds an unsigned simulator `.app` (pre-zipped with `ditto`). A `platforms` input (android/ios/both) picks which jobs run. Artifacts are versioned from package.json (`so-i-quit-<version>-preview.apk` / `-simulator.app.zip` — keep package.json in sync with `build.gradle` versionName + iOS `MARKETING_VERSION`). **iOS artifact is a deliberate zip-in-zip**: GitHub wraps every artifact download in a zip AND its wrapper strips exec permissions (which breaks `.app` bundles), so the job ships its own `ditto` zip — download = `so-i-quit-<v>-simulator.zip` → `so-i-quit-<v>-simulator.app.zip` → `App.app` (unzip twice). Signed iOS device/IPA builds need an Apple Developer account + certificate secrets (not yet wired)
...
- **iOS sizing differs per iOS version** (18: empty inputs can collapse — WebKit bug 198959; 26: redesigned, oversized controls). `main.css` carries an iOS-scoped block (`@supports (-webkit-touch-callout: none)`) that zeroes the `::-webkit-datetime-edit*` shadow-DOM padding, pins `width/min-width/max-width` + `min-height: 2.75rem`, and standardizes `line-height`. **Never `appearance: none` on iOS** — it disables tap-to-open the picker
...
- **iOS launch screen:** Apple always shows the system launch screen (no OEM problem). The `LaunchScreen.storyboard` (generated by `cap add ios`) references the `Splash` imageset with `scaleAspectFill`; `@capacitor/assets generate --ios` fills it (light + dark from `splash.svg`/`splash-dark.svg`) and updates the `Contents.json`. If a min-duration hold is wanted later, `@capacitor/splash-screen`'s `launchShowDuration` works on iOS — no custom VC needed (the plugin was removed from Android; do not re-add for Android)
...
- Android build variants: `debug`, `preview` (debug-keystore-signed for sideload/QA), `release` — `mobile:apk:preview`/`mobile:apk:release` call gradle directly. iOS has no variants: the CI builds an unsigned simulator `.app`; device/IPA builds need Apple signing (secrets) — see `mobile-preview.yml`
