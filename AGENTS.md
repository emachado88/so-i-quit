...
- **`.github/workflows/mobile-preview.yml`** — CD preview installers, `workflow_dispatch` only (previews are on-demand — `ci` already gates PRs and macOS runner minutes are expensive): **android** job (ubuntu) builds `assemblePreview` APK (debug-signed, sideloadable); **ios** job (macos-15) builds an unsigned simulator `.app` (zipped). A `platforms` input (android/ios/both) picks which jobs run. Artifacts are versioned from package.json (`so-i-quit-<version>-preview.apk` / `-simulator.zip` — keep package.json in sync with `build.gradle` versionName + iOS `MARKETING_VERSION`). Signed iOS device/IPA builds need an Apple Developer account + certificate secrets (not yet wired)
