...
- **`.github/workflows/mobile-preview.yml`** — CD preview installers on `pull_request` + `workflow_dispatch`: **android** job (ubuntu) builds `assemblePreview` APK (debug-signed, sideloadable); **ios** job (macos-15) builds an unsigned simulator `.app` (zipped). Artifacts land in the run's Summary page. Signed iOS device/IPA builds need an Apple Developer account + certificate secrets (not yet wired)
